...
void run_motors(int * error) {
		...
		if(*error < -3000) {
			...
			set_motors(0, (speed/2) );
		} else if (*error > -1500 && *error < 1500) { 
			set_motors(speed, speed);
		} else if (*error > 1500) {
			set_motors((speed/2), 0);		
		} else if(*error > 3000) {
			set_motors(speed, 0);
			left_led(1);
			right_led(1);
...
int main() {
	...
	int error = 0;
	int oldError = 0;   
	int time = 0;		//for derivative calculation
	int oldTime = 0;	//ditto
	int i;
	long integral = 0;
	long derivative = 0;
	...
	while(1) {
		...
		error = line_position(sensors,minv,maxv);
/*		time = clock();
		
		//compute derivative
		derivative = ((error-oldError)/(time-oldTime));
		oldTime = time;
		oldError = error; 		
*/		
		run_motors(&error);
