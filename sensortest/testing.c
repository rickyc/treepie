...
//  	time = millis();
    ...
    delta = (position - prev_position)/11; // 4000/20 200
    integral += position; // tracks long running position offset
    offset = position/positionDivisor + delta/2;// + integral/30000;
		
		
		
    if (run == 1) {
    	
    	if (position > 1000 || position < -1000 && counter == 0 && DELAY == 0) {
      	rotation -= 30;
      	counter = 5;
      	DELAY = 1;
      	play_from_program_space(beep_button_top);
...
//    
    ...
    //display_bars(sensorsclear(),minv,maxv);
    delay_ms(10);
//    time = millis()-time;
 //   clear();
 //   print_long(time);
